# llmlab Development Guide

## What This Project Is
llmlab is a local-first Python CLI tool that forecasts LLM API costs.
It uses adaptive exponential smoothing (upgrading to a 3-model ensemble)
on daily spend data to predict total project cost.

## Architecture Rules
- All source code lives in llmlab/ (flat layout, not src/)
- CLI commands are in llmlab/commands/ — each is a thin wrapper calling core logic
...
5. .llmlab.toml uses relative paths (path = "."). Never write absolute paths to config files.
...
- Don't use except Exception: pass — always log errors to ~/.llmlab/error.log

## Key Commands
pip install -e ".[dev,forecast]"   # Install for development
pytest tests/ -v                    # Run all tests
pytest tests/benchmarks/ -v         # Run accuracy benchmarks
ruff check llmlab/ tests/           # Lint
llmlab demo                         # See it working with sample data
